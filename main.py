...
def main():
    ...
    random_search_tuner = kt.RandomSearch(
        hypermodel=build_model, objective="val_mse", max_trials=_max_trials, executions_per_trial=_executions_per_trial,
        overwrite=True,
...
def get_callbacks():
    ...
    if _early_stopping:
        early_stopping = keras.callbacks.EarlyStopping(patience=_patience,
                                                       restore_best_weights=True)  # goes back to best weights
...
def build_model(hp):  # hp Hyper parameters
    ...
    n_hidden = hp.Int("n_hidden", min_value=_min_layer_count, max_value=_max_layer_count,
                      default=_default_layer_count)  # Value calculated by search hyper parameters(hp)
    n_neurons = hp.Int("n_neurons", min_value=_min_neuron_count, max_value=_max_neuron_count)
    learning_rate = hp.Float("learning_rate", min_value=1e-4, max_value=1e-2,
                             sampling="log")

    optimizer = keras.optimizers.SGD(
        learning_rate=learning_rate)  # SGD Gradiant decent Aims to lowest value and adjusted learning rate
    ...
    for i in range(n_hidden):  # adds layers based upon numbers of neurons
        model.add(keras.layers.Dense(n_neurons,
                                     activation="relu"))  # Best activaton function for hiden layers #gives good performance and easy to train #alternitive : sigmoid
...
def set_up_data_skitlearn():
    ...
    X_train, X_valid, y_train, y_valid = train_test_split(X_train_full, y_train_full, test_size=0.2,
                                                          random_state=42)  # splits training set soo we have validation
...
if __name__ == "__main__":
    ...
    parser.add_argument("-S", "--save-model", action="store_true",
                        help="Saves model if flag is true | includes checkpoints")
    ...
    parser.add_argument("-te", "--trial-execution", type=int,
                        help=f"numbers of execution pr trial default is {_executions_per_trial}")
    parser.add_argument("--no-early-stoping", action="store_true",
                        help=f"Disables early stoping | Early stopping a regularization technique in machine learning used to prevent overfitting ")
    parser.add_argument("-l", "--layers", type=int, help=f"numbers of default layers is {_max_layer_count}")
